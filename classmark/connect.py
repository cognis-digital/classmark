"""Native cognis-connect emit for classmark — forward findings to any platform.

Maps classmark's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    classmark ... --format json | classmark-emit --to stix
    classmark-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
