from __future__ import annotations

import argparse
import sys

from .exporters import to_console, to_json, to_markdown, to_oscal_skeleton, to_sarif

_FORMATTERS = {
    "console": to_console,
    "json": to_json,
    "markdown": to_markdown,
    "sarif": to_sarif,
    "oscal": to_oscal_skeleton,
}

_SCAN_EXCLUDES = frozenset(
    {"target", "format", "out", "fail_on", "version", "classification"}
)


def make_cli(tool_name, scan_fn, version="0.1.0", extra_args=None):
    """Build and run a standard Cognis-mil CLI for *scan_fn*.

    Unexpected exceptions are caught and printed to stderr with exit code 1 so
    the caller never sees a raw traceback.
    """
    p = argparse.ArgumentParser(
        prog=tool_name,
        description=f"{tool_name} -- Cognis Digital Military/IC ecosystem",
    )
    p.add_argument("target", nargs="?", default=".", help="Path/target")
    p.add_argument(
        "--format",
        choices=list(_FORMATTERS),
        default="console",
    )
    p.add_argument("--out", help="Write output to file")
    p.add_argument(
        "--fail-on",
        choices=["very_high", "high", "moderate", "low", "none"],
        default="none",
    )
    p.add_argument(
        "--classification",
        default="UNCLASSIFIED//FOR PUBLIC RELEASE",
        help="Operator-supplied banner. PLACEHOLDER. Tool does not interpret.",
    )
    p.add_argument(
        "-v", "--version", action="version", version=f"{tool_name} {version}"
    )
    if extra_args:
        for a in extra_args:
            p.add_argument(
                *a["flags"], **{k: v for k, v in a.items() if k != "flags"}
            )

    args = p.parse_args()

    try:
        result = scan_fn(
            args.target,
            **{k: v for k, v in vars(args).items() if k not in _SCAN_EXCLUDES},
        )
    except SystemExit:
        raise  # scan() uses sys.exit(2) for invalid targets -- let it through
    except Exception as exc:
        print(f"{tool_name}: error: {exc}", file=sys.stderr)
        sys.exit(1)

    result.classification_placeholder = args.classification
    if hasattr(result, "finalize") and not result.composite_score:
        result.finalize()

    formatter = _FORMATTERS[args.format]
    try:
        out = formatter(result)
    except Exception as exc:
        print(f"{tool_name}: error formatting output: {exc}", file=sys.stderr)
        sys.exit(1)

    if args.out:
        try:
            with open(args.out, "w", encoding="utf-8") as fh:
                fh.write(out)
            print(f"Wrote {args.out}", file=sys.stderr)
        except OSError as exc:
            print(
                f"{tool_name}: error: could not write output file"
                f" '{args.out}': {exc}",
                file=sys.stderr,
            )
            sys.exit(1)
    else:
        print(out)

    if args.fail_on != "none":
        from .models import Severity

        thresh = {
            "very_high": [Severity.VERY_HIGH],
            "high": [Severity.VERY_HIGH, Severity.HIGH],
            "moderate": [Severity.VERY_HIGH, Severity.HIGH, Severity.MODERATE],
            "low": [
                Severity.VERY_HIGH,
                Severity.HIGH,
                Severity.MODERATE,
                Severity.LOW,
            ],
        }[args.fail_on]
        if any(f.severity in thresh for f in result.findings):
            sys.exit(1)

    sys.exit(0)
